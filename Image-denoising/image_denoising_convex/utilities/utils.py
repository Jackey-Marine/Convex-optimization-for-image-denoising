...
def read_image(filename):
    ...
    for ext in extensions:
        if os.path.isfile(f'./images/{filename}.{ext}'):
            im = Image.open(f'./images/{filename}.{ext}').convert('L')
            print("Reading images...")
            break
        else :
            print('No .jpg or .png images.')
...
def add_salt_pepper_noise(im, s_vs_p=0.5, amount=0.0005):
    '''Adds Salt pepper noise to an image.

    Args:
        s_vs_p (float): salt and pepper rate.
        amount (float): The number of salt pepper noise.
...
def normalize_image(im):
    ...
    max_pixel = np.max(im)
    min_pixel = np.min(im)
    normalized_im = (im - min_pixel) / (max_pixel - min_pixel)
